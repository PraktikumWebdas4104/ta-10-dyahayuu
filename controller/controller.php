<?php
	...
	class controller{
		...
		function update(){
			...
			$prodi = $_POST['prodi'];
			$genre = implode(",",$_POST['film']);
			$wisata = implode(",",$_POST['wst']);
			
			//panggil model update mahasiswa
			$que = $this->model->updateMhs($nim, $nama, $angkatan, $fakultas, $prodi, $genre, $wisata);
		...
		function insert(){
			...
			$que = $this->model->insertMhs($nim, $nama, $angkatan, $fakultas, $prodi,$genre,$wisata);
			header("location:index.php");
		}
